...
namespace HospitexDPP.ViewModels
{
    public class BrandsViewModel : INotifyPropertyChanged
    {
        ...
        public bool IsSingleBrandMode => App.Session != null && !App.Session.IsAdmin && App.Session.IsBrand;
        ...
        private async Task LoadBrandsAsync()
        {
            try
            {
                ...
                {
                    // Brand-only user: create a synthetic brand from session
                    var session = App.Session!;
                    var brand = new BrandSummary
                    {
                        Id = session.BrandId ?? 0,
                        BrandName = session.BrandName ?? "Mitt brand",
                        ApiKey = session.BrandKey
                    };
                    Brands.Add(brand);
                    StatusText = brand.BrandName;
                    SelectedBrand = brand;
                    return;
                }
