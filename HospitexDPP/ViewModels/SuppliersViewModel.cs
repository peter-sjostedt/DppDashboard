...
namespace HospitexDPP.ViewModels
{
    public class SuppliersViewModel : INotifyPropertyChanged
    {
        ...
        public bool IsSingleSupplierMode => App.Session != null && !App.Session.IsAdmin && App.Session.IsSupplier;

        private async Task LoadSuppliersAsync()
        {
            try
            {
                if (IsSingleSupplierMode)
                {
                    // Supplier-only user: create a synthetic supplier from session
                    var session = App.Session!;
                    var supplier = new SupplierDetail
                    {
                        Id = session.SupplierId ?? 0,
                        SupplierName = session.SupplierName ?? "Min supplier",
                        ApiKey = session.SupplierKey
                    };
                    Suppliers.Add(supplier);
                    StatusText = supplier.SupplierName;
